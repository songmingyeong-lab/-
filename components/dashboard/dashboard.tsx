...
export function Dashboard({ data, areas = [] }: { data: DashboardData; areas?: Array<{ slug: string; label: string }> }) {
  ...
  return (
    <main className="dashboard-shell">
      ...
      <div className="content">
        {areas.length > 0 && <AreaSelector areas={areas} selected={data.area.slug} />}
        ...
        <section className="overview" aria-labelledby="overview-title">
          ...
          <div className="location-box"><div><strong>{data.area.cityName} · {data.area.districtName} {data.area.administrativeDongName}</strong><br /><small>{data.area.scope}</small></div></div>
        </section>
        ...
        <section className="methodology" aria-labelledby="methodology-title">
          ...
          <p>본 지표는 주민 만족도를 직접 측정한 값이 아니라, 공공데이터를 기반으로 {data.area.administrativeDongName}의 주거환경, 생활 불편, 상권 변화, 지역활력, 공동체 활동과 거점시설 상태를 보조적으로 확인하기 위한 Proxy입니다. 공식 도시재생 성과평가와 주민 설문 결과를 함께 고려해야 합니다.</p>
