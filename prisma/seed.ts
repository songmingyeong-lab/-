...
const sources = [
  ...
  { code: "income-consumption", name: "서울시 상권분석서비스(소비-행정동)", serviceId: "OA-22166", serviceName: "VwsmAdstrdNcmCnsmpW", sourceUrl: "https://data.seoul.go.kr/dataList/OA-22166/S/1/datasetView.do", updateCycle: "discontinued", collectionCycle: "quarterly", geographicUnit: "administrative_dong", codeType: "administrative", filterField: "ADSTRD_CD" },
...
const indicators = [
  ...
  { code: "monthly_average_income", sourceCode: "income-consumption", name: "월 평균 소득", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "원/월", direction: FavorableDirection.HIGHER_IS_BETTER, aggregation: "공식 MT_AVRG_INCOME_AMT 제공값(2026-05-13 컬럼 삭제)", comparison: "same_district_other_dongs_mean", stale: 3650, defaultStatus: ObservationStatus.RESTRICTED_DATA, statusMessage: "서울시 상권분석서비스의 월 평균 소득 데이터는 원자료 수급이 2020년에 중단되어 최신화가 불가능하며, 서울시는 2026년 5월 13일 해당 컬럼을 공식 API에서 삭제했습니다.", proxy: "공식 행정동 소득 원자료가 삭제되어 원자료·비교평균·비교율·지표점수를 산출하지 않으며 결측값을 대체하지 않습니다." },
  ...
  { code: "median_monthly_rent", sourceCode: "rental-transaction", name: "주거 월세 중위값", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "만원", direction: FavorableDirection.LOWER_IS_BETTER, aggregation: "대상 법정동의 최신 계약월 월세 거래 중 RTFE가 0보다 큰 자료의 중위값", comparison: "같은 자치구 다른 법정동 평균", stale: 45, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "대상 법정동 주거용 월세 계약의 공개 표본이며 상가 임대료나 전체 임대주택 시세를 뜻하지 않습니다." },
...
async function main() {
  ...
  for (const source of sources) {
    const enabled = ["living-population", "building-register", "commercial-store", "income-consumption", "resident-population", "floating-population", "rental-transaction", "estimated-sales", "road-construction", "noise-complaint", "public-program"].includes(source.code);
