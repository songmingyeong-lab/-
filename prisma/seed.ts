...
const indicators = [
  ...
  { code: "vacant_house_count", sourceCode: "vacant-house", name: "확인된 빈집 수", category: "주거환경", areaGroup: IndicatorArea.HOUSING_ENVIRONMENT, unit: "호", direction: FavorableDirection.LOWER_IS_BETTER, aggregation: "공식 빈집 조사자료의 대상 지역 집계", comparison: "previous_period", stale: 365, defaultStatus: ObservationStatus.MANUAL_VERIFICATION_REQUIRED, statusMessage: "현재 연결된 구로구 빈집현황 파일은 다른 자치구를 지원하지 않으며 공개 단위도 추가 확인이 필요합니다.", proxy: "건축물대장만으로 실제 빈집 여부를 판정하지 않습니다." },
  ...
  { code: "opening_count", sourceCode: "commercial-store", name: "개업 점포 수", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "개", direction: FavorableDirection.HIGHER_IS_BETTER, aggregation: "업종별 OPBIZ_STOR_CO 합계", comparison: "same_district_other_dongs_mean", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "해당 분기 업종별 개업 점포 수 합계이며 개별 점포의 생존 여부를 뜻하지 않습니다." },
  { code: "opening_rate", sourceCode: "commercial-store", name: "개업률", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "%", direction: FavorableDirection.HIGHER_IS_BETTER, aggregation: "공식 업종별 OPBIZ_RT를 전체 점포 수로 가중 집계하며 공식률 부재 시 개업 점포 수 / 전체 점포 수 × 100", comparison: "같은 자치구 다른 행정동 평균", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "점포 진입 변화를 보여주며 개별 점포의 지속가능성을 직접 뜻하지 않습니다." },
  { code: "closing_count", sourceCode: "commercial-store", name: "폐업 점포 수", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "개", direction: FavorableDirection.LOWER_IS_BETTER, aggregation: "업종별 CLSBIZ_STOR_CO 합계", comparison: "same_district_other_dongs_mean", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "해당 분기 업종별 폐업 점포 수 합계이며 폐업 사유나 상권 쇠퇴를 직접 뜻하지 않습니다." },
  { code: "closing_rate", sourceCode: "commercial-store", name: "폐업률", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "%", direction: FavorableDirection.LOWER_IS_BETTER, aggregation: "공식 업종별 CLSBIZ_RT를 전체 점포 수로 가중 집계하며 공식률 부재 시 폐업 점포 수 / 전체 점포 수 × 100", comparison: "같은 자치구 다른 행정동 평균", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "점포 이탈 변화를 보여주며 폐업 사유나 도시재생 사업 성패를 직접 뜻하지 않습니다." },
  ...
  { code: "street_floating_population_density", sourceCode: "floating-population", name: "길 단위 유동인구 밀도", category: "활력·혼잡", areaGroup: IndicatorArea.VITALITY_CONGESTION, unit: "명(합계 대체)", direction: FavorableDirection.CONTEXT_DEPENDENT, aggregation: "도로길이·면적·유효구간 수를 확보하지 못한 경우 우선순위 4인 행정동 길 단위 유동인구 합계 사용", comparison: "같은 자치구 다른 행정동 평균", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "정규화 분모가 없는 합계 대체점수이므로 지역 크기의 영향을 받을 수 있습니다." },
  { code: "floating_population_concentration", sourceCode: "floating-population", name: "시간대별 유동인구 집중도", category: "활력·혼잡", areaGroup: IndicatorArea.VITALITY_CONGESTION, unit: "%", direction: FavorableDirection.CONTEXT_DEPENDENT, aggregation: "6개 시간대 중 최대 유동인구 / 6개 시간대 합계 × 100", comparison: "같은 자치구 다른 행정동 평균", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "최대 시간대가 전체 시간대 유동인구에서 차지하는 비중이며 체감 혼잡도를 직접 뜻하지 않습니다." },
  { code: "street_floating_population_total", sourceCode: "floating-population", name: "길 단위 유동인구 합계", category: "활력·혼잡", areaGroup: IndicatorArea.VITALITY_CONGESTION, unit: "명", direction: FavorableDirection.NEUTRAL, aggregation: "공식 TOT_FLPOP_CO 제공값", comparison: "같은 자치구 다른 행정동 평균", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "점수에 직접 반영하지 않는 길 단위 유동인구 원자료 합계입니다." },
  { code: "floating_population_by_weekday", sourceCode: "floating-population", name: "요일별 길 단위 유동인구", category: "활력·혼잡", areaGroup: IndicatorArea.VITALITY_CONGESTION, unit: "명", direction: FavorableDirection.NEUTRAL, aggregation: "요일별 공식 유동인구 제공값", comparison: "차트 정보", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "요일별 추세 차트용 참고자료이며 점수에 반영하지 않습니다." },
...
async function main() {
  ...
  for (const source of sources) {
    const enabled = ["living-population", "building-register", "commercial-store", "commercial-rent-market", "income-consumption", "resident-population", "floating-population", "rental-transaction", "estimated-sales", "road-construction", "noise-complaint", "public-program"].includes(source.code);
  ...
  await prisma.indicatorDefinition.updateMany({
    where: { code: { in: ["floating_population", "peak_floating_time_band"] } },
    data: { active: false },
  });
}
