...
const indicators = [
  ...
  { code: "median_monthly_rent", sourceCode: "rental-transaction", name: "주거 월세 중위값", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "만원", direction: FavorableDirection.CONTEXT_DEPENDENT, aggregation: "가리봉동 법정동의 최신 계약월 월세 거래 중 RTFE가 0보다 큰 자료의 중위값", comparison: "previous_month", stale: 45, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "가리봉동 주거용 월세 계약의 공개 표본이며 상가 임대료나 전체 임대주택 시세를 뜻하지 않습니다." },
  { code: "estimated_sales", sourceCode: "estimated-sales", name: "당월 추정매출", category: "상권 변화", areaGroup: IndicatorArea.COMMERCIAL_CHANGE, unit: "원", direction: FavorableDirection.CONTEXT_DEPENDENT, aggregation: "행정동 업종별 THSMON_SELNG_AMT 합계", comparison: "previous_quarter", stale: 120, defaultStatus: ObservationStatus.EMPTY, statusMessage: null, proxy: "모형 기반 추정치이며 실제 전체 매출이나 주민 소득을 뜻하지 않습니다." },
...
async function main() {
  ...
  for (const source of sources) {
    const enabled = ["living-population", "building-register", "commercial-store", "floating-population", "rental-transaction", "estimated-sales"].includes(source.code);
