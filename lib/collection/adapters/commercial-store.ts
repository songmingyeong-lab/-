...
export const commercialStoreAdapter: SourceAdapter = {
  ...
  async collect(context) {
    ...
    return {
      sourceCode: this.code, status: "success", recordsRead: data.rows.length + (previousData?.rows.length ?? 0) + residentData.rows.length, recordsSaved: 6, recordsSkipped: data.rows.length - rows.length,
      indicators: [
        ...
        { ...common, code: "opening_count", name: "개업 점포 수", value: summary.openedCount, previousValue: previousSummary?.openedCount ?? null, unit: "개", statusMessage: null, favorableDirection: "HIGHER_IS_BETTER" as const, proxyDescription: "해당 분기 업종별 개업 점포 수의 합계이며 개별 점포의 생존 여부를 뜻하지 않습니다.", spatialComparison: comparisonData("개", (item) => item.openedCount) },
        { ...common, code: "opening_rate", name: "개업률", value: summary.openingRate, previousValue: previousSummary?.openingRate ?? null, unit: "%", statusMessage: summary.openingRateSource === "OFFICIAL_WEIGHTED" ? "서울시 공식 업종별 개업률을 업종별 전체 점포 수로 가중 집계했습니다." : `공식률이 없어 개업 점포 ${summary.openedCount}개를 전체 점포 ${summary.storeCount}개로 나누어 계산했습니다.`, proxyDescription: "업종별 비율의 단순평균이 아닌 전체 점포 수 가중 집계값입니다.", spatialComparison: comparisonData("%", (item) => item.openingRate) },
        { ...common, code: "closing_count", name: "폐업 점포 수", value: summary.closedCount, previousValue: previousSummary?.closedCount ?? null, unit: "개", statusMessage: null, favorableDirection: "LOWER_IS_BETTER" as const, proxyDescription: "해당 분기 업종별 폐업 점포 수의 합계이며 폐업 사유나 상권 쇠퇴를 직접 뜻하지 않습니다.", spatialComparison: comparisonData("개", (item) => item.closedCount) },
        { ...common, code: "closing_rate", name: "폐업률", value: summary.closingRate, previousValue: previousSummary?.closingRate ?? null, unit: "%", statusMessage: summary.closingRateSource === "OFFICIAL_WEIGHTED" ? "서울시 공식 업종별 폐업률을 업종별 전체 점포 수로 가중 집계했습니다." : `공식률이 없어 폐업 점포 ${summary.closedCount}개를 전체 점포 ${summary.storeCount}개로 나누어 계산했습니다.`, proxyDescription: "업종별 비율의 단순평균이 아닌 전체 점포 수 가중 집계값입니다.", spatialComparison: comparisonData("%", (item) => item.closingRate) },
      ],
